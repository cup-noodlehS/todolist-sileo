      ...
      <div class="card-inner">
        ...
        <ul class="list mb-3">
          ...
          >
            ...
            <span>{{ item.text }}</span>
            <div class="list-item-delete" @click="removeItem(item.pk)">X</div>
          </li>
        </ul>
...
const Item = new sileo.Model("todoapp", "items");
...
export default {
  ...
  methods: {
    sortItems() {
      this.todoItems = this.items.filter((item) => !item.completed);
      this.doneItems = this.items.filter((item) => item.completed);
    },
    async getItems() {
      await Item.objects
        .filter()
    ...
    async addItem() {
      await Item.objects
        .create({ text: this.newItem, completed: false })
        .then((res) => {
          console.log(res);
        });
    ...
    async removeItem(itemID) {
      this.items = this.items.filter((item) => item.pk !== itemID);
      this.sortItems();
      await Item.objects.delete({ pk: itemID }).then((res) => {
        console.log(res);
      });
    },
  },
